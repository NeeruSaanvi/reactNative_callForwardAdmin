...
class CDRImport extends React.Component {
  ...
  parse_cdr = () => {
    ...
    let data = {
      ...
      billingSecond: parseInt(this.state.billing_type),
      withLrn: this.state.withLrn,
  ...
  fillNpanxx = () => {
    this.props.callApi(RestApi.dipCDR, response => {}, {
      "withLrn": this.state.withLrn,
      "billingSecond": parseInt(this.state.billing_type)
    })
  };
            ...
            <CardBody>
              ...
              <FormGroup row>
                <Col lg="3" className="row">
                  <Label className="col-6 text-right">Column Delimeter:</Label>
                ...
                <Col lg="3" className="row">
                  <Label className="col-6 text-right">Import Type:</Label>
                ...
                <Col lg="3" className="row">
                  <Label className="col-6 text-right">Billing Type:</Label>
                  <Input type="select" name="billing_type" className="col-6 form-control-sm" onChange={this.handleChange}>
                    <option value="1">1 / 1</option>
                    <option value="6">6 / 6</option>
                  </Input>
                </Col>
                <Col lg="3" className="row">
                  <Label className="col-6 text-right"> Has Column Header:<Input type="checkbox" name="header" onChange={(ev) => {this.setState({header: ev.target.checked})}} className="ml-3"/></Label>
                </Col>
              </FormGroup>
              <Card>
                <CardHeader>
              ...
              <Row>
                <Col lg="2" className="text-right">
                  <FormGroup check inline>
                    <Label check>
                      <Input type="checkbox" onChange={(evt) => {this.setState({withLrn: evt.target.checked})}}/> DIP with LRN
                    </Label>
                  </FormGroup>
                </Col>
                <Col lg="4">
                  <Button size="md" color="primary" className="font-weight-bold" onClick={this.fillNpanxx} disabled={!this.state.withLrn}>Fill NPANXX</Button>
                </Col>
    ...
    return <Col lg="4" className="row text-right">
      ...
      <Col lg="7">
        <Input type="select" name={name} className="form-control-sm mt-4" onChange={this.handleChange} value={value}>
          <option value="NONE">NONE</option>
          {this.state.columns.map((i, d) => {return <option key={i} value={d}>{i}</option>})}
        </Input>
      </Col>
    </Col>
  }
