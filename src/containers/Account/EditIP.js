  ...
  render() {
    ...
    return (
      <Card>
        ...
        <Collapse isOpen={!this.state.isCollapsed}>
          <CardBody>
            <Form action="" method="post" className="form-horizontal">
              ...
              <ul>
                {ips.map((ip) => {
                    return <li>{ip} {!this.props.disabled && <a color="link" onClick={() => this.removeIp(ip)}>
                    { this.props.isSuperAdmin &&  <i className="fa fa-times"/> }</a>}</li>;
                  }
                )}
              </ul>
              <FormGroup row>
                ...
                <Col xs="12" md="9">
                  <InputGroup>
                    <Input type="email" id="ip" name="ip" placeholder="IP Address"
                           onChange={this.handleChange} value={this.state.ip} className="form-control" disabled={this.props.disabled || !this.props.isSuperAdmin}/>
                    <InputGroupAddon addonType="append">
                      <Button type="button" color="primary" onClick={this.addIp} disabled={this.props.disabled || !this.props.isSuperAdmin}>ADD</Button>
                    </InputGroupAddon>
                  </InputGroup>
        ...
        {this.props.hasFooter && this.props.isSuperAdmin ? (<CardFormFooter resetHandler={this.reset} updateHandler={this.update}/>) : undefined}
      </Card>
    );
  }
