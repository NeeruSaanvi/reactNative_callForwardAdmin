...
class MainInfo extends React.Component {
  ...
  render() {
    ...
    let disabled = ((this.props.data.id && !this.props.profile.isSuperAdmin) || this.props.disabled) ? true : false;
    return(
      <Card>
        ...
        <Collapse isOpen={!this.state.isCollapsed}>
          <CardBody>
            <Form action="" method="post" className="form-horizontal">
              {this.addInput("username", "Username", "text", data.username, this.handleChange('username'), data.usernameError, disabled)}
              <FormGroup row>
                <Col sm="3">
                  <Label htmlFor="company">Company: </Label>
                </Col>
                <Col xs="12" sm="9">
                  <Input type="select" name="compId" id="main-info-compId" onChange={this.handleCompanyChange} value={data.compId}>
                      {this.props.company.map(c => (<option key={c.id} value={c.id}>{c.name}</option>))}
                   </Input>
                </Col>
              </FormGroup>
              <FormGroup row>
  ...
  handleCompanyChange = (evt) => {
    if (this.props.handleCompanyChange) {
  ...
  update = () => {
    ...
    if (!emailError)
      ...
      if(!data.compId)
        data.compId = this.props.company[0].id;
