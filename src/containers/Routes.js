...
const NewCPRReport = React.lazy(() => import('./NewCPRReport'));
...
const getRoutes = (privileges) => {
  ...
  if (privileges.includes(Privileges.ReadCompany) || privileges.includes(Privileges.WriteCompany)) {
    result.push({ path: '/configuration/company', name: 'Company', component: Company });
    if (!route.component)
      route.component = (props) => <Redirect to="/configuration/company"/>
  }
  ...
  if (privileges.includes(Privileges.LergImport)) {
    result.push({ path: '/cprgen/lerg_import', name: 'RGLE Import', component: LergImport});
    if (!route.component)
      route.component = (props) => <Redirect to="/cprgen/lerg_import"/>
  }
  if (privileges.includes(Privileges.ViewLerg)) {
    result.push({ path: '/cprgen/view_lerg', name: 'View RGLE', component: ViewLerg});
    if (!route.component)
  ...
  if (privileges.includes(Privileges.NewCPRReport)) {
    result.push({ path: '/cprgen/new_cprgen_report', name: 'CPRgen Report', component: NewCPRReport});
    if (!route.component)
      route.component = (props) => <Redirect to="/cprgen/new_cprgen_report"/>
  }
