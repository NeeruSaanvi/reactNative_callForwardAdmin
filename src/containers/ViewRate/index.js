...
class ViewRate extends Component {
  ...
  subscribeNotification = () => {
    ...
    eventSource.onmessage = (event) => {
      ...
      try {
        ...
        if (data.message && data.category === NotificationCategory.CprGenRateDeckImportDone){
          this.fetchData();
        }
      } catch(ex){}
    };
  ...
  columns = [
    ...
    },
    {
      ...
      accessor: 'eff_date',
      Cell: props => {
        return <div className="text-center">{props.value.split("?").join("")}</div>}
    },
    {
      Header: 'LATA',
      accessor: 'lata',
    ...
    },
    {
      ...
      filterable: true,
      Cell: props => <div className="text-center">{props.value && parseInt(props.value)}</div>
    },
    {
...
export default withAuthApiLoadingNotification(ViewRate);
