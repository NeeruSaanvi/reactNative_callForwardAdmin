...
class Users extends Component {
  constructor(props) {
    ...
    this.switchUserFormRef = React.createRef();
  }
    ...
    },
    {
      ...
      Cell: (row) => {
        return <div className="text-center">
          {row.row.is_active ? <Badge className="mr-1" color="success">Active</Badge> : <Badge className="mr-1" color="secondary">Inactive</Badge>}
        </div>
      }
    },
    {
    ...
    },
    {
      ...
      Cell: (row) => {
        ...
        return <div className="text-center">
          ...
          <span color="link" className="mr-2" onClick={() => this.deleteUser(id)}><i className="fa fa-trash"/></span>
          {/* <span color="link" onClick={() => this.switchUser(row.original.username)}>
  ...
  switchUser = async (username) => {
    await this.setState({"switchUsername": username});
    console.log(this.switchUserFormRef.current)
    this.switchUserFormRef.current.submit();
  }
    ...
    return (
      <Fragment>
        ...
        <EditUserModal
          ...
          isSuperAdmin={this.props.profile.isSuperAdmin}
        />
        {/* <div>
          <form ref={this.switchUserFormRef} method="GET" action="@{/impersonate}">
            <input value={this.state.switchUsername} type="text" name="username"/>
  ...
  updateMainHandler = () => {
    var data = this.state.modal.user.main;
    if(!data.compId || data.compId == "null" || data.compId == 0) {
  ...
  updatePasswordHandler = () => {
    // Validation Password
    if(this.state.modal.user.password) {
  ...
  updateUser = (id) => {
    ...
    if(!user.main.compId)
      user.main.compId = this.state.company[0].id;
    ...
    if (id === 0) {
      this.props.callApi(RestApi.createUser, response => {
        ...
        username: user.main.username, compId: user.main.compId, role: user.main.roleId ? user.main.roleId : this.props.roles[0].id, email: user.main.email,
...
export default connect(state => (
  {roles: state.auth.profile.roles, profile: state.auth.profile}
))(withLoadingAndNotification(Users))
